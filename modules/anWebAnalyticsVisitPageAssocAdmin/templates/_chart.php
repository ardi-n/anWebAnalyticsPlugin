...
echo _open('table.wa-chart-data');
	...
	echo _open('tfoot');
		...
		foreach ($data as $k => $item) {
			...
			$label = wa_format_label($item, $k>0 ? $data[$k-1] : null, $groupby, false, 'datestart');
			$longLabel = wa_format_label($item, $k>0 ? $data[$k-1] : null, $groupby, true, 'datestart');
			
			echo _tag('th', array('data-long-label' => __('%c% new pageviews on %d%', array('%c%' => $item['COUNT'], '%d%' => $longLabel))), $showAxisLabel ? $label : '');
		}
		echo _close('tr');
	echo _close('tfoot');
